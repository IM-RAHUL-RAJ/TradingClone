...
export class TradeComponent {
  ...
  filterTable(){
      return this.company.filter(item=>
        item.instrument.instrumentDescription.toLowerCase().includes(this.search.toLowerCase())
      );
    }
    ...
    onBuy(){
      ...
      const action='Buy';
      const time=new Date();
      const shares:any=(document.getElementById('number_of_shares') as HTMLInputElement).value;
      const price:any=document.getElementById('price')?.textContent;
      const balance=parseInt(shares)*parseInt(price);
      // console.log(companyName,date,action,time,shares,price,balance)
